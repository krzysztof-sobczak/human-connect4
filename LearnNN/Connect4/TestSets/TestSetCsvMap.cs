...
{
    public class TestSetCsvMap : CsvClassMap<TestSetCsv>
    {
        ...
        public override void CreateMap()
        {
            ...
            Map(m => m.BestColumns).ConvertUsing(row =>
                {
                    var fieldValue = row.GetField(68);
                    int bestColumnValue;
                    if(int.TryParse(fieldValue, out bestColumnValue))
                    {
                        return new List<int>() { bestColumnValue };
                    }
                    else
                    {
                        List<int> bestColumns = new List<int>();
                        var splittedBestColumns = fieldValue.Replace('{', ' ').Replace('}', ' ').Split(',');
                        foreach (string bestColumn in splittedBestColumns)
                        {
                            int value = 0;
                            int.TryParse(bestColumn, out value);
                            bestColumns.Add(value);
                        }
                        return bestColumns;
                    }
                }
